...
def setup_model_dataset(args):
    if args.dataset == "cifar10":
        ...
        if args.imagenet_arch:
            ...
            if args.chenyaofo:
                model = torch.hub.load("chenyaofo/pytorch-cifar-models", f"{args.dataset}_{args.arch}", pretrained=True)
            else:
        ...
        train_full_loader, val_loader, _ = vggface2_dataloaders(
            batch_size=args.batch_size, data_dir=args.data, num_workers=args.workers
        )
        marked_loader, _, test_loader = vggface2_dataloaders(
            batch_size=args.batch_size,
            data_dir=args.data,
            num_workers=args.workers,
            class_to_replace=args.class_to_replace,
        ...
        classes = 256
        train_full_loader, val_loader, _ = caltech256_dataloaders(
            batch_size=args.batch_size, data_dir=args.data_dir, num_workers = args.num_workers
            )
        marked_loader, _, test_loader = caltech256_dataloaders(
            batch_size=args.batch_size,
