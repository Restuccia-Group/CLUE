...
def set_deterministic(seed=42, cudnn=False):
    random.seed(seed)
    np.random.seed(seed)
...
def get_in_channels(data_code):
    in_ch = -1
    if data_code in ['mnist', 'fmnist']:
        in_ch = 1
    elif data_code in ['cifar10', 'cifar100', 'imagenet', "textures"]:
        in_ch = 3
    else:
...
def get_in_dimensions(data_code):
    ...
    if data_code in ['mnist', 'fmnist']:
        in_dim = (32, 32)
    elif data_code in ['cifar10', 'cifar100']:
        in_dim = (32, 32)
    elif data_code=='imagenet':
        in_dim = (254, 254)
    elif code=="textures":
        in_dim = 128
    else:
...
def get_normalization_params(data_code):
    if data_code in ['mnist', 'fmnist']:
        mean, std = None, None
...
def get_num_classes(data_code):
    if data_code in ['svhn', 'mnist', 'fmnist', 'cifar10']:
        return 10
    elif data_code == "cifar100":
        return 100
    elif data_code == "imagenet":
...
def get_accuracy_epoch(model, dataloader, device):
...
def get_hsic_epoch(config_dict, model, dataloader, device):
    """ Computes the hsic
    """
    acc = []
    loss = []
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(dataloader):
            data = data.to(device)
            target = target.to(device)
            output = model(data)   #### #### What is this hiddens?
...
def eval_robust_epoch(model, dataloader, config_dict, device):
    #############################################################
    #    This function generates the attacked image on the go   #
    acc = []
    acc5 = []
    model = model.to(device)
    # device = next(model.parameters()).device
